...
public class DaoCidade implements IDaoCidade {
	...
	public void salvar(Cidade cidade) throws DaoException {
		...
		try {
			...
			e.printStackTrace();
			throw new DaoException("ERRO AO INSERIR CIDADE - CONTACTE A EQUIPE RESPONSÁVEL");
	...
	public Cidade buscarPorId(int id) throws DaoException {
		...
		try {
			...
			if(resultSet.next()) {
				cidade = new Cidade(resultSet.getInt(1), resultSet.getString(2), resultSet.getString(3), resultSet.getString(4));
			}else{
