...
public class DaoMovimentacao implements IDaoMovimentacao {
	...
	public Movimentacao buscarPorId(int id) throws DaoException {
		try {
			...
			if(resultSet.next()) {
				movimentacao = new Movimentacao(resultSet.getInt("id"), resultSet.getString("descricao"), resultSet.getString("situacao"), 
						resultSet.getString("tipo"),Ultil.converterDataParaString(resultSet.getDate("data_movimentacao")), resultSet.getDouble("valor"),resultSet.getBoolean("status"));
